...
def __generate_data() -> dict:
    output = {}
    try:
        r = requests.get(url='http://localhost:5100/data')
        ...
        if not str(r.status_code).startswith('2'):
            ...
            try:
                output = r.json()
                output['dbms'] = 'new_company'
                output['table'] = 'py_monitor'
            except:
                pass
    return output
...
def publish_batch(data_generators, conn, db_name, auth, timeout, exception, batch_size):
    payloads = [__generate_data() for _ in range(batch_size)]
    try:
...
def main():
    ...
    try:
        while total_rows < args.total_rows:
            payload = [__generate_data() for _ in range(args.batch_size)]
