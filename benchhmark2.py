...
def main():
    ...
    def publish_payload(conn, payload, auth, timeout, exception, mode):
        try:
            publish_via_put(conn=conn, payload=payload, auth=auth, timeout=timeout, exception=exception, mode=mode)
        except Exception as e:
            if exception:
                print(f"Error publishing batch: {e}")
    ...
    try:
        ...
        while args.total_rows == 0 or total_rows < args.total_rows:
            ...
            # If exceeding the total rows, truncate the payload
            if args.total_rows > 0 and total_rows + len(payload) > args.total_rows:
                payload = payload[:args.total_rows - total_rows]
            ...
            if args.single_insert:
                ...
                publish_payload(args.conn, payload, args.auth, args.timeout, args.exception, args.mode)
                PROCESSING_INSERTS.append(time.time() - put_start_time)

            if total_rows % args.batch_size == 0:
                end_time = time.time()
                BATCH_INSERTS.append(end_time - start_time)
                start_time = end_time
                print(f"\t- Batch Number: {total_rows // args.batch_size} | Process Time: {BATCH_INSERTS[-1]:.3f} seconds")
    ...
    if args.single_insert and payloads:
        ...
        put_start_time = time.time()
        publish_payload(args.conn, payloads, args.auth, args.timeout, args.exception, args.mode)
        PROCESSING_INSERTS.append(time.time() - put_start_time)

    print(f"""----------------------------------------------------------------------------------------------------------
Benchmark completed with {total_rows:,} rows inserted | Total Processing Time: {time.time() - init_time:.3f} seconds
\tData Generator Time: {sum(BATCH_INSERTS):.3f} seconds | AVG: {sum(BATCH_INSERTS)/len(BATCH_INSERTS):.3f} | MIN: {min(BATCH_INSERTS):.3f} | MAX: {max(BATCH_INSERTS):.3f}  
\tInsertion Time: {sum(PROCESSING_INSERTS):.3f} seconds | AVG: {sum(PROCESSING_INSERTS)/len(PROCESSING_INSERTS):.3f} | MIN: {min(PROCESSING_INSERTS):.3f} | MAX: {max(PROCESSING_INSERTS):.3f}
----------------------------------------------------------------------------------------------------------
""")
