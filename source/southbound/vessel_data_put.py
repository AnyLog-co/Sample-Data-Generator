        ...
        for id_index, group in enumerate(vessel_files[side]):
            ...
            for fname in vessel_files[side][group]:
                ...
                if not row:
                    ...
                    break
                row.update(general_params)
                payload.append(row)
            # print(len(payload), payload)
            publish_data(method="PUT", conn=conn, payload=payload, db_name="anotherpeak", table_name=group)

if __name__ == "__main__":
    conn = RestClient(conn="50.116.20.125:32149", auth=(), timeout=30)
    main(method="PUT", conn=conn, publish_topics=None, db_name="anotherpeak", iterations=1)