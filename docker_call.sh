...
if [[ ${SLEEP} ]] ; then SLEEP=`echo --slep ${SLEEP}` ; fi
...
if [[ ! ${EXCEPTION} == true ]] ; then
  python3 data_generator.py ${CONN} ${GENERATOR} ${SAVE} ${DBMS} ${REPEAT} ${SLEEP} ${BATCH_REPEAT} ${BATCH_SLEEP} ${TIMEZONE} ${AUTHENTICATION} ${TIMEOUT} ${TOPIC} ${TOKEN} ${TAG}
else
  python3 data_generator.py ${CONN} ${GENERATOR} ${SAVE} ${DBMS} ${REPEAT} ${SLEEP} ${BATCH_REPEAT} ${BATCH_SLEEP} ${TIMEZONE} ${AUTHENTICATION} ${TIMEOUT} ${TOPIC} ${TOKEN} ${TAG}
fi
